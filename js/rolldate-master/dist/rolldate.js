...
//   styleInject(css);
  ...
  Rolldate.prototype = {
      ...
      baseData: function baseData() {
          return {
              ...
              opts: { //插件默认配置
                  ...
                  minStep: 1,
                  trigger: 'tap',
                  lang: { title: '选择日期', cancel: '取消', clear: '清空', confirm: '确认', year: '年', month: '月', day: '日', hour: '时', min: '分', sec: '秒' }
              }
          };
      },
      ...
      event: function event() {
          ...
          _this.tap(clear, function () {
            var config = _this.config,
                el = void 0;
            if (config.clear) {
                var flag = config.clear.call(_this, date);
                if (flag === false) {
                    return false;
                } else if (flag) {
                    date = flag;
      ...
      destroy: function destroy(flag) {
          ...
          if (flag && config.clear) {
              config.clear.call(_this);
          }
          setTimeout(function () {
